...
## ❌ S5 Momentum Leader — TESTED AND REJECTED (strategy5.py)

Built to beat S3; it does not. **Not wired live.** Kept as a documented negative
so the idea is not re-attempted from scratch.

Design: absolute momentum (stand down when SPY < its 200-day SMA) + relative
momentum (blended 3 / 6 / 12-1 month return, skipping the last month for
short-term reversal) + a realised-volatility penalty. The premise was that a
*ranking* strategy degrades more gracefully than a *timing* one, and that the
regime gate would cut the second-half damage that killed S1 and S2.
...
**The regime gate did not work.** Second-half losses accrued while SPY was still
above its own 200-day line — momentum stopped paying without the index rolling
over, so an index-level filter never fired.

### The finding that outlives the strategy
Three independent trend-following entries — S1 (pullback), S2 (breakout), S5
(momentum ranking) — all collapse in the second half of this 3-year window. The
one mean-reversion strategy, S3, does not. **That is a property of the window,
not of any individual signal.** A fourth trend-following variant should not be
expected to behave differently; if the goal is to beat S3, look at
mean-reversion variants, exit rules, or position sizing instead.

Also established: sizing cannot manufacture edge. An S3 risk sweep scaled return
and drawdown together (0.5% risk → +17.4%/12.4%DD, 1.0% → +34.5%/19.7%DD,
2.0% → +53.7%/25.3%DD), leaving the return/drawdown ratio far below SPY's.

Reproduce: `python3 backtest.py --strategy s5 --years 3 --scan-every 3 --robustness`
