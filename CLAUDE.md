...
## 🔬 S4 Options Flow — RESEARCH ONLY (s4_logger.py, options_flow.py)

**S4 does not trade.** It records a signal and waits. There is no
`/api/s4/execute` and there should not be one until the log shows an edge.

**Premise:** lopsided call activity in the weekly expiry is a bullish tell.
Untested — and untestable the way S1/S2/S3 were, because `backtest.py` replays
daily OHLCV and there is no historical option chain in this stack (yfinance
does not serve it; Alpaca options history needs OPRA + a paid plan). The only
route to evidence is to gather it forward.

**Ranks on call SHARE, not raw call volume.** Measured 2026-08-12 on the 472
universe, the top of the raw-volume ranking was MU (48.3% call share), QQQ
(44.0%) and PLTR (46.6%) — three names where PUTS outnumbered calls. Raw volume
ranks liquidity, returns the same mega caps daily, and can point at bearish flow
while looking bullish.

**Single stocks only.** The 17 index/sector ETFs in the universe are excluded
(`options_flow.ETF_SYMBOLS`) — ETF options are bought as hedges and broad-market
expressions, so their flow says nothing about a company.
...
sample of the universe is recorded over the same windows — without it a "+2% in
5 days" result is unreadable. Forward returns at 1/5/20 sessions, measured from
the recording day's close (never revised once written).

Runs at **16:15 ET each weekday**; `GET /api/s4/flow`; state in
`s4_flow_log.json` on the volume. Needs ~30+ observations per cohort before the
comparison means anything — `summary()["verdict"]` says so until it has them.

**Day-1 observation (2026-08-12):** all 5 candidates failed the tech filter on
volume — `vol_ratio` 0.53–0.80, i.e. the most call-heavy names traded on
*below*-average equity volume.
